...
- Memory graph projection and synapse-map rendering (`memory-graph.ts`)
- WebSocket UI contracts with response-length controls (`ui-system.ts`)
- Workshop tool/sync integration surfaces (`workshop.ts`)
- Analytics event + automation rule engines (`analytics-automation.ts`)
- Cognitive system integrator wiring helper (`cognitive-integration.ts`)
- Extended knowledge graph entity/relationship models (`extended-knowledge-graph.ts`)
- Conversation flow manager with intent/sentiment/topic state tracking (`conversation-flow.ts`)
- Notification template and delivery manager (`notification-system.ts`)
- Structured event logging and query surfaces (`event-logging.ts`)
...
- `MemoryGraphRenderer` (2D graph projection + HTML/JSON export)
- `AgentChatServer` + `ResponseLengthConfig` (public UI runtime controls)
- `WorkshopIntegration` (tool registry and sync contract)
- `AgentAnalytics` + `AutomationEngine` (observability + event rules)
- `integrateCognitiveSystems` (single-call runtime wiring)
- `KnowledgeGraph` (entity/relationship/cluster graph store)
- `ConversationFlowManager` (turn/state/sentiment/intent management)
- `NotificationManager` + `NotificationTemplateRegistry` (template-driven delivery)
- `EventLogger` (structured audit/event stream with query filters)
